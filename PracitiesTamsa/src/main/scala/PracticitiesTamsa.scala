...
class PracticitiesTamsa extends Serializable{
    ...
    private def crearPartida(a:Usuario) : Unit = {
        ...
        nPartidasIds += 1
        aux.gm = a.nombre
        println("Ingrese el nombre de su partida")
        aux.name = readLine()
        println("Ingrese el lore (contexto) de su partida")
        aux.lore = readLine()
    ...
    private def entrarPartida(a:Usuario) : Unit = {
        ...
        var aux : Int = readInt()
        if(aux != -1){
            println(partidas(aux).name)
            println(partidas(aux).lore)
    ...
    private def imprimirLobby() : Unit = {
        for((s,v) <- lobby ){
                println( "\n Informacion de la partida con id: "+ s + "\n")
                var partida : PartidaHistoria = partidas(s)
                println(partida.name)
